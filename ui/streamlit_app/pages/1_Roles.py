...
def create_role_section():
    def body():
        with st.form("create_role"):
            ...
            wireguard = st.checkbox("Wireguard туннель", value=False)
            job_control = st.checkbox("Контроль заданий", value=False)
            interfaces_raw = st.text_area("JSON интерфейсов (опционально)", value="", height=120)
...
def clone_role_section():
    def body():
        with st.form("clone_role"):
            name = st.text_input("Исходная роль")
            new_name = st.text_input("Имя копии")
