...
def fetch_job_logs_by_name(job_name):
    logs = api_request("GET", f"/logs/jobs/{job_name}")
    if isinstance(logs, list):
        return logs
    return []
...
def render_logs():
    ...
    if st.session_state.logs_page_size not in page_sizes:
        st.session_state.logs_page_size = 25

    with st.container(key="card"):
        previous_filter = st.session_state.logs_job_filter
        previous_page_size = st.session_state.logs_page_size

        c1, c2, c3 = st.columns([4, 2, 1])
        c1.selectbox(
            "Job",
            options=filter_options,
            index=filter_options.index(st.session_state.logs_job_filter),
            key="logs_job_filter",
        )
        c2.selectbox(
            "Rows per page",
            options=page_sizes,
            index=page_sizes.index(st.session_state.logs_page_size),
            key="logs_page_size",
        )
        refresh = c3.button("Refresh", use_container_width=True)

        if st.session_state.logs_job_filter != previous_filter:
            st.session_state.logs_page = 1
            st.rerun()
        if st.session_state.logs_page_size != previous_page_size:
            st.session_state.logs_page = 1
            st.rerun()
        if refresh:
            st.rerun()
        ...
        rows.append(
            {
                ...
                "std_out": str(entry.get("std_out", "") or ""),
                "std_err": str(entry.get("std_err", "") or ""),
            }
        )

    rows.sort(key=lambda row: (row["timestamp"], row["name"]), reverse=True)
    ...
    total = len(rows)
    page_size = st.session_state.logs_page_size
    total_pages = (total + page_size - 1) // page_size
    if st.session_state.logs_page > total_pages:
        st.session_state.logs_page = total_pages
    if st.session_state.logs_page < 1:
        st.session_state.logs_page = 1
    ...
    top = st.columns([2, 2, 3, 1, 1])
    top[0].metric("Total logs", total)
    top[1].metric("Page", f"{page}/{total_pages}")
    top[2].write(f"Showing **{start + 1}-{end}** of **{total}**")
    if top[3].button("Prev", use_container_width=True, disabled=page <= 1):
        st.session_state.logs_page = page - 1
        st.rerun()
    if top[4].button("Next", use_container_width=True, disabled=page >= total_pages):
        st.session_state.logs_page = page + 1
        st.rerun()
    ...
    if pd is not None:
        ...
        if selected_idx is not None:
            selected_log = page_rows[selected_idx]
            with st.container(key="card"):
                st.markdown("#### Log details")
                st.write(f"Job: **{selected_log['name']}**")
                st.write(f"Client: **{selected_log['client']}**")
                st.write(f"Timestamp: **{selected_log['timestamp']}**")
                st.write(f"Status: **{selected_log['status_code']}**")
                st.markdown("Command")
                st.code(selected_log["command"], language="bash")
                st.markdown("STDOUT")
                st.code(selected_log["std_out"], language="text")
                st.markdown("STDERR")
                st.code(selected_log["std_err"], language="text")
    else:
        for idx, row in enumerate(page_rows, start=start + 1):
            with st.expander(f"{idx}. {row['timestamp']} | {row['name']} | {row['status_code']}"):
                st.write(f"Client: **{row['client']}**")
                st.markdown("Command")
                st.code(row["command"], language="bash")
                st.markdown("STDOUT")
                st.code(row["std_out"], language="text")
                st.markdown("STDERR")
                st.code(row["std_err"], language="text")
