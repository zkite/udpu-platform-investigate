...
def create_vbce_section():
    def body():
        mode = st.radio("Режим", ["Форма", "Raw JSON"], horizontal=True)
        if mode == "Raw JSON":
...
def manage_vbce_section():
    def body():
        with st.form("vbce_get"):
            name = st.text_input("Имя VBCE для получения")
        ...
        with st.form("vbce_patch"):
            target = st.text_input("Имя VBCE для обновления")
            max_users = st.text_input("Максимум пользователей")
